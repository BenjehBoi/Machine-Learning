...
usefulData = table(pClasses, pSex, ageGroups, pPort, pSurvival);
...
modelspec = 'pSurvival ~ pClasses*sortedSex*ageGroups - pClasses:sortedSex:ageGroups';
logistic_regression = fitglm(usefulData,modelspec, 'Distribution','binomial');
prediction = predict(logistic_regression,usefulData(:,1:end-1));
prediction = round(prediction);

error = confusionmat(pSurvival,prediction);
error_analysis = trace(error)/sum(error, 'all')*100;
disp(error_analysis + "% match to ground truth table of training file.");
